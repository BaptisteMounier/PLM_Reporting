...
- Création de tous les blocks nécessaire à PLM
  - Séparation de leur ajout dans la liste disponible suivant
    l'exercice en cours
...
- Modification de l'ajout de l'interface de Blockly
  - A mettre dans des fichiers extérieur et jouer avec les appels
    plutôt que de le mettre en dure dans l'html afin de simplifier la
    "confiuration" de Blockly
...
- Remplacer l'aspect de l'éditeur de source actuel par l'interface de
  ...
  - Création de blocs correspondant aux fonctions déjà fournis par
    l'enseignant
- Générer du code python à envoyer au serveur lors des executions de
  l'éditeur de source à travers Blockly
...
- Modification de l'affichage de la zone du code pour laisser la place
  à Blockly
- Se pencher sur la correspondance entre l'exercice en cours avec ses
  langages supportés et Blockly
- Regarder comment récupérer le code généré par Blockly (probablement
  en python mais à définir)
** 8 avril
*** Activité
- Documentation
...
*** Objectif
- Test de fonctionnement des différents outils déjà existants pour les
  associer pour un rendu les comprennants
